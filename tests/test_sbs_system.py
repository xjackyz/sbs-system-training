...
from src.analysis.sbs_analyzer import SBSAnalyzer, SBSSteps
...
class TestSBSSystem(unittest.TestCase):
    ...
    def test_9_auto_save_state(self):
        """测试自动保存状态"""
        trainer = SBSTrainer('config/training_config.yaml')
        trainer.auto_save_state()
        checkpoint_path = trainer.checkpoint_path + '/latest.pt'
        self.assertTrue(os.path.exists(checkpoint_path), "检查点文件未保存")

    def test_10_resume_training(self):
        """测试恢复训练"""
        trainer = SBSTrainer('config/training_config.yaml')
        trainer.auto_save_state()  # 首先保存状态
        trainer.resume_training()  # 恢复训练
        self.assertEqual(trainer.current_epoch, 0, "恢复的轮次不正确")

    def test_11_error_handling(self):
        """测试错误处理"""
        trainer = SBSTrainer('config/training_config.yaml')
        with self.assertRaises(ValueError):
            trainer.train()  # 这里可以模拟一个错误
